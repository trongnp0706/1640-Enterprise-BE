...
func (q *Queries) GetIdeaByAcademicyear(ctx context.Context, academicYear string) ([]int64, error) {
	rows, err := q.db.QueryContext(ctx, getIdeaByAcademicyear, academicYear)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
type GetMostPopularIdeasRow struct {
	ID            string         `json:"id"`
	Title         string         `json:"title"`
	Content       string         `json:"content"`
	ViewCount     int32          `json:"view_count"`
...
func (q *Queries) GetMostPopularIdeas(ctx context.Context, arg GetMostPopularIdeasParams) ([]GetMostPopularIdeasRow, error) {
	rows, err := q.db.QueryContext(ctx, getMostPopularIdeas, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetMostPopularIdeasRow
	for rows.Next() {
		var i GetMostPopularIdeasRow
		if err := rows.Scan(
			&i.ID,
			&i.Title,
			&i.Content,
...
type GetMostViewedIdeasRow struct {
	ID            string         `json:"id"`
	Title         string         `json:"title"`
	Content       string         `json:"content"`
	ViewCount     int32          `json:"view_count"`
...
func (q *Queries) GetMostViewedIdeas(ctx context.Context, arg GetMostViewedIdeasParams) ([]GetMostViewedIdeasRow, error) {
	rows, err := q.db.QueryContext(ctx, getMostViewedIdeas, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetMostViewedIdeasRow
	for rows.Next() {
		var i GetMostViewedIdeasRow
		if err := rows.Scan(
			&i.ID,
			&i.Title,
			&i.Content,
...
const increaseView = `-- name: IncreaseView :one
UPDATE ideas
...
func (q *Queries) IncreaseView(ctx context.Context, id string) (int32, error) {
	row := q.db.QueryRowContext(ctx, increaseView, id)
